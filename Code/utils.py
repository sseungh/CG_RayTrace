...
# For checking if the rays intersect with the environment
def square_intersecting(rectangle_points, origin, direction):
    ...
    # Check if the line and the plane of the rectangle are parallel.
    if np.dot(normal_vector, direction) == 0:
        return np.inf, False  # 선과 사각형 평면이 평행하면 교차하지 않습니다. if parallel, no intersection.
